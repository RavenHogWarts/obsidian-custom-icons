	...
	settings: {
		...
		fileExplorer: {
			...
			extensions: {
				...
				placeholder:
					"Введите расширения, поддерживается пакетный ввод (напр. .xdb .js)...",
		...
		tabHeader: {
			...
			enable: {
				name: "Включить функцию",
				desc: "Настроить значки для заголовков вкладок рабочей области (вкладки боковой панели и редактора) по типу представления. Логика «по типу»: все вкладки одного типа используют один значок (например, назначение markdown затронет все вкладки документов). Исходные значки скрываются, но не удаляются, и восстанавливаются при отключении.",
			},
			mapping: {
				name: "Сопоставление типов",
				desc: "Назначьте значки типам представлений (data-type); также можно задать значок на месте, щёлкнув правой кнопкой по значку вкладки. Настроенные записи перечислены здесь.",
	...
	view: {
		CustomIconLib: {
			...
			pack: {
				...
				installed:
					"Установлено иконок: {count:number}, вступает в силу сразу",
				...
				npmModal: {
					...
					globPlaceholder:
						"glob-путь к SVG (напр. icons/outline/*.svg)",
					versionPlaceholder:
						"Версия (необязательно, по умолчанию latest)",
				...
				redownloadHint:
					"Повторно загружает набор из источника и перезаписывает локальную копию (текущую версию может заменить более новая). Вступает в силу сразу; при ошибке загрузки существующий набор сохраняется.",
				versionLabel: "Версия",
