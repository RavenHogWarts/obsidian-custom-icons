...
describe("uniqueIconId", () => {
	...
	test("被占用时从 -1 开始递增", () => {
		expect(uniqueIconId("star", new Set(["star"]))).toBe("star-1");
		expect(uniqueIconId("star", new Set(["star", "star-1"]))).toBe(
			"star-2",
		);
	});
	...
	test("跳过中间已占用的编号", () => {
		expect(
			uniqueIconId("star", new Set(["star", "star-1", "star-2"])),
		).toBe("star-3");
	});
	...
	test("同批次连续调用不会撞名（调用方回填 taken）", () => {
		...
		expect(first).toBe("star-1");
		expect(second).toBe("star-2");
	});

	test("已有 -1 后缀的名字照常处理", () => {
		expect(uniqueIconId("star-1", new Set(["star-1"]))).toBe("star-1-1");
	});
});
