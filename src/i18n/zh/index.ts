...
const zh = {
	settings: {
		...
		tabHeader: {
			...
			mapping: {
				name: "类型映射",
				desc: "为视图类型（data-type）分配兜底图标，作用于未单独配置的所有同类型标签；也可在标签页右键图标处就地设置。此处列出已配置项。",
				selectType: "选择视图类型…",
				addTooltip: "添加映射",
			...
			tabs: {
				name: "单标签覆盖",
				desc: "为单个标签（类型 + 标签名）单独分配图标，优先于类型映射。点击刷新从当前打开的标签抓取候选；右键标签页图标也可就地设置。文件标签以文件名标识（不同目录的同名文件共享），视图标签随界面语言。",
				selectTab: "选择标签…",
				addTooltip: "添加覆盖",
