	...
	settings: {
		...
		fileExplorer: {
			...
			extensions: {
				...
				placeholder: "Введите расширения, поддерживается пакетный ввод (напр. .xdb .js)...",
		...
		tabHeader: {
			...
			mapping: {
				...
				desc: "Назначьте запасные значки типам представлений (data-type); они применяются ко всем вкладкам типа без индивидуальной настройки; также можно задать значок на месте, щёлкнув правой кнопкой по значку вкладки. Из-за ограничения Obsidian щелчок правой кнопкой по неактивной вкладке показывает только исходные пункты меню — сначала щёлкните левой кнопкой, чтобы активировать вкладку, и пункт «Задать значок» появится в контекстном меню. Настроенные записи перечислены здесь.",
				selectType: "Выберите тип представления…",
				addTooltip: "Добавить сопоставление",
				fetchTooltip: "Получить типы из открытых вкладок",
				noneFound: "Сопоставления типов ещё не настроены",
				resetTooltip: "Удалить",
			},
			tabs: {
				name: "Индивидуальные значки вкладок",
				desc: "Назначьте значки отдельным вкладкам (тип + название вкладки); они имеют приоритет над сопоставлением типов. Нажмите «Обновить», чтобы получить кандидатов из открытых вкладок; также можно задать значок на месте, щёлкнув правой кнопкой по значку вкладки (у неактивных вкладок в контекстном меню только исходные пункты; пункт «Задать значок» появляется после активации вкладки). Вкладки файлов определяются по имени файла (одноимённые файлы из разных папок совпадают), названия вкладок представлений зависят от языка интерфейса.",
